...
public class Main {
	...
	static Matrix[] weighs=new Matrix[type_num];	//写入文件
	
	static double[] history=new double[100];	//写入文件
	static double[] prediction_score=new double[test_instance_num];
	static int[] prediction_type=new int[test_instance_num];	//写入文件
	...
	public static void main(String[] args) throws IOException {
		...
		FileWriter fw=new FileWriter("D:\\Zhanyang\\Classification\\results\\history.txt");
		for(int i=0;i<100;i++){
			fw.write(""+history[i]+"\n");
		}
		fw.close();
	...
	public static void training(int type, int times) throws IOException{
		...
		FileWriter fw=new FileWriter("D:\\Zhanyang\\Classification\\results\\weighs.txt");
	...
	public static void predict_type() throws IOException{
		...
		for(int i=0;i<test_instance_num;i++){
			for(int j=0;j<7;j++){
				Matrix score=(test_attrs.getMatrix(new int[]{i}, 0, attr_num-1)).times(weighs[j]);
	...
	public static void calculate_F1() throws IOException{
		double[] Macro_f1s=new double[7];	//写入文件
		double[] precisions=new double[7];	//写入文件
		double[] recalls=new double[7];	//写入文件
